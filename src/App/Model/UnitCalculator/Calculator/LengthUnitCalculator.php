...
class LengthUnitCalculator implements IUnitCalculator
{
    ...
    public function collectValues(string $message): array
    {
        ...
        preg_match_all(sprintf('/([0-9.,]+)\s?(%s)\b/i', implode('|', $unitsToFind)), $message, $matches);
    ...
    private static function calculateNewUnit(UnitCalculatorValue $unitCalculatorValue, float $newUnitConversionRate): int
    {
        ...
        return $baseValue / $newUnitConversionRate;
    }

    private function loadConfig(): void
    {
        $this->config = ConfigLoader::load(__DIR__ . '/../../../../../config/unit-calculator.json')['length-unit-calculator'];
    }
}